            ...
            <div class="card alert">
                ...
                <div class="card-body">
                    <table class="table table-responsive table-hover ">
                        ...
                        <c:forEach var="p" items="${goods}">
                            <tr>
                                <td style="text-align: center;display: none;" class="color-success">${p.id}</td>
                                <td style="text-align: center;" class="color-success">${p.goodscode}</td>
                                <td style="text-align: center;" class="color-success">${p.goodsname}</td>
                                <td style="text-align: center;" class="color-success">${p.goodstype}</td>
                                <td style="text-align: center;" class="color-success">${p.goodspecs}</td>
                                <td style="text-align: center;" class="color-success">${p.goodsbrand}</td>
                                <td style="text-align: center;width: 150px;height: 150px" class="color-success"><img style="width: 100%;height: 100%;" src="${pageContext.request.contextPath }/${p.goodspicture}"></td>
                                <td style="text-align: center;" class="color-success">${p.saleprice}</td>
                                <td style="text-align: center;" class="color-success">${p.count}</td>
                                <td style="text-align: center;" class="color-success">${p.isonsale}</td>
                                <td style="text-align: center;" class="color-success">${p.goodsdescribe}</td>
                                <td>
                                    <button class="btn btn-primary btn-xs" onclick="forword('${pageContext.request.contextPath }/Goods?method=GoodsCount&id=${p.id}&count=1')">采购<i class="fa fa-pencil"></i></button>
                                    <button class="btn btn-primary btn-xs" onclick="forword('${pageContext.request.contextPath }/Goods?method=updateGoods&id=${p.id}&count=1')">修改<i class="fa fa-pencil"></i></button>
                                    <button class="btn btn-danger btn-xs" onclick="forword('${pageContext.request.contextPath }/Goods?method=delGoods&id=${p.id}')">删除<i class="fa fa-trash-o "></i></button>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    ...
                    <button class="btn btn-block btn-success" onclick="forword('${pageContext.request.contextPath }/view/admin/form/Goods.jsp')">添加<i class="fa fa-check"></i></button>
                    </div>
                </div>
            </div>
        </div><!-- /# column -->
